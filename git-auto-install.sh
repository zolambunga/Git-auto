...
if [ -f "$TARGET" ]; then
    echo "    > Arquivo já existe. Não será sobrescrito."
else
    echo "    > Arquivo não existe. Criando agora."
    cat > "$TARGET" << 'END'
...
if grep -Fxq "$BLOCK_START" "$BASHRC"; then
    echo "    > Bloco já existe no .bashrc. Nada a fazer."
else
    echo "    > Bloco não encontrado. Adicionando..."
...
echo "    source ~/.bashrc"
echo ""
echo "Depois disso, o comando 'git-auto' estará disponível."
echo ""
